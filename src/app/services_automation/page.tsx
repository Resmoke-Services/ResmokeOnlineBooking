...
function ServiceCategoryCard({ title, imageUrl, imageAlt, imageHint, href, category }: ServiceCategoryCardProps) {
  ...
  const handleCategoryClick = () => {
    setServicePath(["AUTOMATION", category]);
  };
...
export default function ServicesAutomation() {
  return (
     <div className="min-h-screen flex flex-col bg-background text-foreground">
        <ServiceSelectionTracker selections={["AUTOMATION"]} />
        <BookingHeader />
        <main className="flex-grow">
            <section id="services" className="py-24">
              <div className="container mx-auto px-4">
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground uppercase">AUTOMATION SERVICES</h2>
